...
export function decodeInstruction(hex) {
    ...
    if (opcode === 0x00) {
        switch (funct) {
            ...
            case 0x00:
                return `sll $${regName(rd)}, $${regName(rt)}, ${shamt}`;
            case 0x02:
                return `srl $${regName(rd)}, $${regName(rt)}, ${shamt}`;
            case 0x03:
                return `sra $${regName(rd)}, $${regName(rt)}, ${shamt}`;
            case 0x04:
                return `sllv $${regName(rd)}, $${regName(rt)}, $${regName(rs)}`;
            case 0x06:
                return `srlv $${regName(rd)}, $${regName(rt)}, $${regName(rs)}`;
            case 0x07:
                return `srav $${regName(rd)}, $${regName(rt)}, $${regName(rs)}`;
            case 0x08:
            ...
            case 0x0c:
                return 'syscall';
            default:
                return `R-type funct 0x${funct.toString(16)}`;
        }
    }
    ...
    switch (opcode) {
        case 0x08:
            return `addi $${regName(rt)}, $${regName(rs)}, ${signExtend(imm)}`;
        case 0x09:
            return `addiu $${regName(rt)}, $${regName(rs)}, ${signExtend(imm)}`;
        case 0x0c:
            return `andi $${regName(rt)}, $${regName(rs)}, 0x${imm.toString(16)}`;
        case 0x0d:
            return `ori $${regName(rt)}, $${regName(rs)}, 0x${imm.toString(16)}`;
        case 0x0e:
            return `xori $${regName(rt)}, $${regName(rs)}, 0x${imm.toString(16)}`;
        case 0x0f:
            return `lui $${regName(rt)}, 0x${imm.toString(16)}`;
        case 0x04:
            return `beq $${regName(rs)}, $${regName(rt)}, offset ${signExtend(imm)}`;
        case 0x05:
            return `bne $${regName(rs)}, $${regName(rt)}, offset ${signExtend(imm)}`;
        case 0x06:
            return `blez $${regName(rs)}, offset ${signExtend(imm)}`;
        case 0x07:
            return `bgtz $${regName(rs)}, offset ${signExtend(imm)}`;
        case 0x01:
            switch (rt) {
                case 0x00:
                    return `bltz $${regName(rs)}, offset ${signExtend(imm)}`;
                case 0x01:
                    return `bgez $${regName(rs)}, offset ${signExtend(imm)}`;
                default:
                    return `REGIMM ${rt}`;
            }
        case 0x20:
            return `lb $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x21:
            return `lh $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x23:
            return `lw $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x24:
            return `lbu $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x25:
            return `lhu $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x28:
            return `sb $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x29:
            return `sh $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
        case 0x2b:
            return `sw $${regName(rt)}, ${signExtend(imm)}($${regName(rs)})`;
    }
...
export function TextSegmentDisplay({
                                       textDump, breakpoints = new Set(), toggleBreakpoint = () => {
    }, currentLine = null, showAscii = false
                                   }) {
...
export function DataSegmentDisplay({dataDump, showAscii = false}) {
    ...
    const filledDataValues = [...initialDataValues, ...Array(15 * 8 - initialDataValues.length).fill('00000000')];
    const tableValues = Array.from({length: 15}, (_, rowIndex) =>
        filledDataValues.slice(rowIndex * 8, (rowIndex + 1) * 8)
    );
        ...
        <div style={{overflowX: 'auto', width: '100%'}}>
            <table style={{
                ...
                <tr>
                    ...
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+0)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+4)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+8)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+c)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+10)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+14)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+18)</th>
                    <th style={{border: '1px solid grey', padding: '5px'}}>Value (+1c)</th>
                </tr>
                </thead>
                <tbody>
